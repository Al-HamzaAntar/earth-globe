import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  ar: {
    translation: {
      title: "الكرة الأرضية التفاعلية",
      subtitle: "كرة أرضية تفاعلية ومتجاوبة تضم جميع دول العالم.",
      country: "الدولة",
      capital: "العاصمة",
      'globe.title': 'خريطة دول العالم',
      'globe.instructions': 'اسحب للدوران. قم بالتمرير للتكبير. حرك الماوس فوق دولة لرؤية اسمها وعاصمتها.',
      'globe.loading': 'تحميل الخريطة...',
      'globe.capital': 'العاصمة',
      'globe.unknown': 'غير معروف',
      'search.placeholder': 'ابحث عن دولة...',
      'search.button': 'بحث',
      'search.success': 'تم العثور على الدولة',
      'search.found': 'تم العثور على الدولة بنجاح على الكرة الأرضية.',
      'search.error': 'لم يتم العثور على الدولة',
      'search.notFound': 'لا يمكن العثور على الدولة المحددة. يرجى المحاولة باسم مختلف.',
      'countryInfo.loading': 'جاري التحميل...',
      'countryInfo.capital': 'العاصمة',
      'countryInfo.population': 'عدد السكان',
      'countryInfo.area': 'المساحة',
      'countryInfo.languages': 'اللغات',
      'countryInfo.currency': 'العملة',
      'countryInfo.region': 'المنطقة',
      'countryInfo.unknown': 'غير معروف',
     'countryInfo.km2': 'كم²',
     'countryInfo.adminDivisions': 'التقسيمات الإدارية',
     'countryInfo.showDivisions': 'عرض التقسيمات الإدارية',
     'countryInfo.hideDivisions': 'إخفاء التقسيمات الإدارية',
     'countryInfo.noDivisions': 'لا تتوفر بيانات التقسيمات الإدارية لهذه الدولة',
      // Regions in Arabic
      'regions': {
        'Africa': 'أفريقيا',
        'Americas': 'الأمريكتين',
        'Asia': 'آسيا',
        'Europe': 'أوروبا',
        'Oceania': 'أوقيانوسيا',
        'Antarctic': 'القارة القطبية الجنوبية',
      },
      // Subregions in Arabic
      'subregions': {
        'Northern Africa': 'شمال أفريقيا',
        'Eastern Africa': 'شرق أفريقيا',
        'Middle Africa': 'وسط أفريقيا',
        'Southern Africa': 'جنوب أفريقيا',
        'Western Africa': 'غرب أفريقيا',
        'Northern Europe': 'شمال أوروبا',
        'Eastern Europe': 'شرق أوروبا',
        'Southern Europe': 'جنوب أوروبا',
        'Western Europe': 'غرب أوروبا',
        'Central Europe': 'وسط أوروبا',
        'Northern America': 'أمريكا الشمالية',
        'Central America': 'أمريكا الوسطى',
        'Caribbean': 'الكاريبي',
        'South America': 'أمريكا الجنوبية',
        'Western Asia': 'غرب آسيا',
        'Central Asia': 'وسط آسيا',
        'Southern Asia': 'جنوب آسيا',
        'South-Eastern Asia': 'جنوب شرق آسيا',
        'Eastern Asia': 'شرق آسيا',
        'Australia and New Zealand': 'أستراليا ونيوزيلندا',
        'Melanesia': 'ميلانيزيا',
        'Micronesia': 'ميكرونيزيا',
        'Polynesia': 'بولينيزيا',
      },
      // Languages in Arabic
      'languages': {
        'Arabic': 'العربية',
        'English': 'الإنجليزية',
        'French': 'الفرنسية',
        'Spanish': 'الإسبانية',
        'Portuguese': 'البرتغالية',
        'German': 'الألمانية',
        'Italian': 'الإيطالية',
        'Russian': 'الروسية',
        'Chinese': 'الصينية',
        'Japanese': 'اليابانية',
        'Korean': 'الكورية',
        'Hindi': 'الهندية',
        'Bengali': 'البنغالية',
        'Urdu': 'الأوردية',
        'Turkish': 'التركية',
        'Persian': 'الفارسية',
        'Hebrew': 'العبرية',
        'Swahili': 'السواحيلية',
        'Amharic': 'الأمهرية',
        'Dutch': 'الهولندية',
        'Greek': 'اليونانية',
        'Polish': 'البولندية',
        'Ukrainian': 'الأوكرانية',
        'Romanian': 'الرومانية',
        'Czech': 'التشيكية',
        'Swedish': 'السويدية',
        'Hungarian': 'المجرية',
        'Serbian': 'الصربية',
        'Croatian': 'الكرواتية',
        'Bulgarian': 'البلغارية',
        'Slovak': 'السلوفاكية',
        'Danish': 'الدنماركية',
        'Finnish': 'الفنلندية',
        'Norwegian': 'النرويجية',
        'Thai': 'التايلاندية',
        'Vietnamese': 'الفيتنامية',
        'Indonesian': 'الإندونيسية',
        'Malay': 'الملايو',
        'Tagalog': 'التاغالوغ',
        'Somali': 'الصومالية',
        'Pashto': 'البشتو',
        'Kurdish': 'الكردية',
        'Berber': 'البربرية',
        'Hausa': 'الهاوسا',
        'Yoruba': 'اليوروبا',
        'Igbo': 'الإيغبو',
        'Zulu': 'الزولو',
        'Afrikaans': 'الأفريقانية',
      },
      // Currencies in Arabic
      'currencies': {
        'Saudi Riyal': 'الريال السعودي',
        'Egyptian Pound': 'الجنيه المصري',
        'Jordanian Dinar': 'الدينار الأردني',
        'Lebanese Pound': 'الليرة اللبنانية',
        'Syrian Pound': 'الليرة السورية',
        'Iraqi Dinar': 'الدينار العراقي',
        'Kuwaiti Dinar': 'الدينار الكويتي',
        'Qatari Riyal': 'الريال القطري',
        'United Arab Emirates Dirham': 'الدرهم الإماراتي',
        'Omani Rial': 'الريال العماني',
        'Bahraini Dinar': 'الدينار البحريني',
        'Yemeni Rial': 'الريال اليمني',
        'Moroccan Dirham': 'الدرهم المغربي',
        'Algerian Dinar': 'الدينار الجزائري',
        'Tunisian Dinar': 'الدينار التونسي',
        'Libyan Dinar': 'الدينار الليبي',
        'Sudanese Pound': 'الجنيه السوداني',
        'United States Dollar': 'الدولار الأمريكي',
        'Euro': 'اليورو',
        'British Pound': 'الجنيه الإسترليني',
        'Japanese Yen': 'الين الياباني',
        'Chinese Yuan': 'اليوان الصيني',
        'Indian Rupee': 'الروبية الهندية',
        'Russian Ruble': 'الروبل الروسي',
        'Brazilian Real': 'الريال البرازيلي',
        'Canadian Dollar': 'الدولار الكندي',
        'Australian Dollar': 'الدولار الأسترالي',
        'Mexican Peso': 'البيزو المكسيكي',
        'South African Rand': 'الراند الجنوب أفريقي',
        'Turkish Lira': 'الليرة التركية',
        'Swiss Franc': 'الفرنك السويسري',
        'South Korean Won': 'الوون الكوري الجنوبي',
        'Nigerian Naira': 'النايرا النيجيرية',
        'Pakistani Rupee': 'الروبية الباكستانية',
        'Indonesian Rupiah': 'الروبية الإندونيسية',
        'Philippine Peso': 'البيزو الفلبيني',
        'Thai Baht': 'البات التايلاندي',
        'Malaysian Ringgit': 'الرينغيت الماليزي',
        'Singapore Dollar': 'الدولار السنغافوري',
        'New Zealand Dollar': 'الدولار النيوزيلندي',
      },
      // Country names in Arabic
      'countries': {
        // Arab World
        'Yemen': 'اليمن',
        'Saudi Arabia': 'المملكة العربية السعودية',
        'Egypt': 'مصر',
        'Jordan': 'الأردن',
        'Lebanon': 'لبنان',
        'Syria': 'سوريا',
        'Iraq': 'العراق',
        'Kuwait': 'الكويت',
        'Qatar': 'قطر',
        'United Arab Emirates': 'الإمارات العربية المتحدة',
        'Oman': 'عُمان',
        'Bahrain': 'البحرين',
        'Palestine': 'فلسطين',
        'Morocco': 'المغرب',
        'Algeria': 'الجزائر',
        'Tunisia': 'تونس',
        'Libya': 'ليبيا',
        'Sudan': 'السودان',
        'Somalia': 'الصومال',
        'Djibouti': 'جيبوتي',
        'Mauritania': 'موريتانيا',
        'Comoros': 'جزر القمر',
        // Major World Powers
        'United States of America': 'الولايات المتحدة الأمريكية',
        'China': 'الصين',
        'Russia': 'روسيا',
        'India': 'الهند',
        'Japan': 'اليابان',
        'Germany': 'ألمانيا',
        'France': 'فرنسا',
        'United Kingdom': 'المملكة المتحدة',
        'Italy': 'إيطاليا',
        'Spain': 'إسبانيا',
        'Canada': 'كندا',
        'Australia': 'أستراليا',
        'Brazil': 'البرازيل',
        'Mexico': 'المكسيك',
        // Africa
        'South Africa': 'جنوب أفريقيا',
        'Nigeria': 'نيجيريا',
        'Kenya': 'كينيا',
        'Ethiopia': 'إثيوبيا',
        'Ghana': 'غانا',
        'Uganda': 'أوغندا',
        'Tanzania': 'تنزانيا',
        'Zimbabwe': 'زيمبابوي',
        'Zambia': 'زامبيا',
        'Botswana': 'بوتسوانا',
        'Namibia': 'ناميبيا',
        'Angola': 'أنغولا',
        'Cameroon': 'الكاميرون',
        'Central African Rep.': 'جمهورية أفريقيا الوسطى',
        'Congo': 'الكونغو',
        'Dem. Rep. Congo': 'جمهورية الكونغو الديمقراطية',
        'Gabon': 'الغابون',
        'Eq. Guinea': 'غينيا الاستوائية',
        'Chad': 'تشاد',
        'Niger': 'النيجر',
        'Mali': 'مالي',
        'Burkina Faso': 'بوركينا فاسو',
        'Senegal': 'السنغال',
        'Gambia': 'غامبيا',
        'Guinea-Bissau': 'غينيا بيساو',
        'Guinea': 'غينيا',
        'Sierra Leone': 'سيراليون',
        'Liberia': 'ليبيريا',
        'Côte d\'Ivoire': 'ساحل العاج',
        'Togo': 'توغو',
        'Benin': 'بنين',
        'Burundi': 'بوروندي',
        'Rwanda': 'رواندا',
        'Malawi': 'مالاوي',
        'Mozambique': 'موزمبيق',
        'eSwatini': 'إسواتيني',
        'Lesotho': 'ليسوتو',
        'Madagascar': 'مدغشقر',
        // Asia
        'Turkey': 'تركيا',
        'Iran': 'إيران',
        'Afghanistan': 'أفغانستان',
        'Pakistan': 'باكستان',
        'Bangladesh': 'بنغلاديش',
        'Sri Lanka': 'سريلانكا',
        'Maldives': 'المالديف',
        'Nepal': 'نيبال',
        'Bhutan': 'بوتان',
        'Myanmar': 'ميانمار',
        'Thailand': 'تايلاند',
        'Cambodia': 'كمبوديا',
        'Laos': 'لاوس',
        'Vietnam': 'فيتنام',
        'Malaysia': 'ماليزيا',
        'Singapore': 'سنغافورة',
        'Brunei': 'بروناي',
        'Indonesia': 'إندونيسيا',
        'Philippines': 'الفلبين',
        'Timor-Leste': 'تيمور الشرقية',
        'Papua New Guinea': 'بابوا غينيا الجديدة',
        'South Korea': 'كوريا الجنوبية',
        'North Korea': 'كوريا الشمالية',
        'Mongolia': 'منغوليا',
        'Kazakhstan': 'كازاخستان',
        'Kyrgyzstan': 'قيرغيزستان',
        'Tajikistan': 'طاجيكستان',
        'Turkmenistan': 'تركمانستان',
        'Uzbekistan': 'أوزبكستان',
        'Armenia': 'أرمينيا',
        'Azerbaijan': 'أذربيجان',
        'Georgia': 'جورجيا',
        'Israel': 'إسرائيل',
        // Europe
        'Poland': 'بولندا',
        'Czech Rep.': 'جمهورية التشيك',
        'Slovakia': 'سلوفاكيا',
        'Hungary': 'المجر',
        'Slovenia': 'سلوفينيا',
        'Croatia': 'كرواتيا',
        'Bosnia and Herz.': 'البوسنة والهرسك',
        'Serbia': 'صربيا',
        'Montenegro': 'الجبل الأسود',
        'Albania': 'ألبانيا',
        'North Macedonia': 'مقدونيا الشمالية',
        'Bulgaria': 'بلغاريا',
        'Romania': 'رومانيا',
        'Moldova': 'مولدوفا',
        'Ukraine': 'أوكرانيا',
        'Belarus': 'بيلاروسيا',
        'Lithuania': 'ليتوانيا',
        'Latvia': 'لاتفيا',
        'Estonia': 'إستونيا',
        'Finland': 'فنلندا',
        'Sweden': 'السويد',
        'Norway': 'النرويج',
        'Denmark': 'الدنمارك',
        'Iceland': 'آيسلندا',
        'Ireland': 'أيرلندا',
        'Netherlands': 'هولندا',
        'Belgium': 'بلجيكا',
        'Luxembourg': 'لوكسمبورغ',
        'Switzerland': 'سويسرا',
        'Austria': 'النمسا',
        'Liechtenstein': 'ليختنشتاين',
        'Monaco': 'موناكو',
        'Andorra': 'أندورا',
        'San Marino': 'سان مارينو',
        'Vatican': 'الفاتيكان',
        'Malta': 'مالطا',
        'Cyprus': 'قبرص',
        'Greece': 'اليونان',
        'Portugal': 'البرتغال',
        // Americas
        'Argentina': 'الأرجنتين',
        'Chile': 'تشيلي',
        'Uruguay': 'أوروغواي',
        'Paraguay': 'باراغواي',
        'Bolivia': 'بوليفيا',
        'Peru': 'بيرو',
        'Ecuador': 'الإكوادور',
        'Colombia': 'كولومبيا',
        'Venezuela': 'فنزويلا',
        'Guyana': 'غيانا',
        'Suriname': 'سورينام',
        'Guatemala': 'غواتيمالا',
        'Belize': 'بليز',
        'El Salvador': 'السلفادور',
        'Honduras': 'هندوراس',
        'Nicaragua': 'نيكاراغوا',
        'Costa Rica': 'كوستاريكا',
        'Panama': 'بنما',
        'Jamaica': 'جامايكا',
        'Haiti': 'هايتي',
        'Dominican Rep.': 'جمهورية الدومينيكان',
        'Cuba': 'كوبا',
        'Bahamas': 'جزر البهاما',
        // Oceania
        'New Zealand': 'نيوزيلندا',
        'Fiji': 'فيجي',
        'Vanuatu': 'فانواتو',
        'Solomon Islands': 'جزر سليمان',
        'Samoa': 'ساموا',
        'Tonga': 'تونغا',
        'Kiribati': 'كيريباتي',
        'Tuvalu': 'توفالو',
        'Nauru': 'ناورو',
        'Palau': 'بالاو',
        'Marshall Islands': 'جزر مارشال',
        'Micronesia': 'ميكرونيزيا',
        // Territories and Dependencies
        'Greenland': 'غرينلاند',
        'Puerto Rico': 'بورتوريكو',
        'Falkland Is.': 'جزر فوكلاند',
        'Fr. S. Antarctic Lands': 'الأراضي الفرنسية الجنوبية والقطبية الجنوبية',
        'W. Sahara': 'الصحراء الغربية'
      },
      // Capitals in Arabic
      'capitals': {
        // Arab World
        'Sana\'a': 'صنعاء',
        'Riyadh': 'الرياض',
        'Cairo': 'القاهرة',
        'Amman': 'عمَّان',
        'Beirut': 'بيروت',
        'Damascus': 'دمشق',
        'Baghdad': 'بغداد',
        'Kuwait City': 'مدينة الكويت',
        'Doha': 'الدوحة',
        'Abu Dhabi': 'أبو ظبي',
        'Muscat': 'مسقط',
        'Manama': 'المنامة',
        'Jerusalem': 'القدس',
        'Ramallah': 'رام الله',
        'Rabat': 'الرباط',
        'Algiers': 'الجزائر',
        'Tunis': 'تونس',
        'Tripoli': 'طرابلس',
        'Khartoum': 'الخرطوم',
        'Mogadishu': 'مقديشو',
        'Djibouti': 'جيبوتي',
        'Nouakchott': 'نواكشوط',
        'Moroni': 'موروني',
        // Major World Powers
        'Washington, D.C.': 'واشنطن العاصمة',
        'Beijing': 'بكين',
        'Moscow': 'موسكو',
        'New Delhi': 'نيودلهي',
        'Tokyo': 'طوكيو',
        'Berlin': 'برلين',
        'Paris': 'باريس',
        'London': 'لندن',
        'Rome': 'روما',
        'Madrid': 'مدريد',
        'Ottawa': 'أوتاوا',
        'Canberra': 'كانبرا',
        'Brasília': 'برازيليا',
        'Mexico City': 'مكسيكو سيتي',
        // Africa
        'Pretoria': 'بريتوريا',
        'Cape Town': 'كيب تاون',
        'Bloemfontein': 'بلومفونتين',
        'Abuja': 'أبوجا',
        'Nairobi': 'نيروبي',
        'Addis Ababa': 'أديس أبابا',
        'Accra': 'أكرا',
        'Kampala': 'كمبالا',
        'Dodoma': 'دودوما',
        'Harare': 'هراري',
        'Lusaka': 'لوساكا',
        'Gaborone': 'غابورون',
        'Windhoek': 'ويندهوك',
        'Luanda': 'لواندا',
        'Yaoundé': 'ياوندي',
        'Bangui': 'بانغي',
        'Brazzaville': 'برازافيل',
        'Kinshasa': 'كينشاسا',
        'Libreville': 'ليبرفيل',
        'Malabo': 'مالابو',
        'N\'Djamena': 'نجامينا',
        'Niamey': 'نيامي',
        'Bamako': 'باماكو',
        'Ouagadougou': 'واغادوغو',
        'Dakar': 'داكار',
        'Banjul': 'بانجول',
        'Bissau': 'بيساو',
        'Conakry': 'كوناكري',
        'Freetown': 'فريتاون',
        'Monrovia': 'مونروفيا',
        'Yamoussoukro': 'ياموسوكرو',
        'Lomé': 'لومي',
        'Porto-Novo': 'بورتو نوفو',
        'Gitega': 'غيتيغا',
        'Kigali': 'كيغالي',
        'Lilongwe': 'ليلونغوي',
        'Maputo': 'مابوتو',
        'Mbabane': 'مبابان',
        'Maseru': 'ماسيرو',
        'Antananarivo': 'أنتاناناريفو',
        // Asia
        'Ankara': 'أنقرة',
        'Tehran': 'طهران',
        'Kabul': 'كابول',
        'Islamabad': 'إسلام آباد',
        'Dhaka': 'دكا',
        'Colombo': 'كولومبو',
        'Malé': 'ماليه',
        'Kathmandu': 'كاتماندو',
        'Thimphu': 'تيمفو',
        'Naypyidaw': 'نايبيداو',
        'Bangkok': 'بانكوك',
        'Phnom Penh': 'بنوم بنه',
        'Vientiane': 'فيينتيان',
        'Hanoi': 'هانوي',
        'Kuala Lumpur': 'كوالالمبور',
        'Singapore': 'سنغافورة',
        'Bandar Seri Begawan': 'بندر سري بيغاوان',
        'Jakarta': 'جاكرتا',
        'Manila': 'مانيلا',
        'Dili': 'ديلي',
        'Port Moresby': 'بورت مورسبي',
        'Seoul': 'سيول',
        'Pyongyang': 'بيونغ يانغ',
        'Ulaanbaatar': 'أولان باتور',
        'Nur-Sultan': 'نور سلطان',
        'Bishkek': 'بيشكيك',
        'Dushanbe': 'دوشانبي',
        'Ashgabat': 'عشق آباد',
        'Tashkent': 'طشقند',
        'Yerevan': 'يريفان',
        'Baku': 'باكو',
        'Tbilisi': 'تبليسي',
        'Tel Aviv': 'تل أبيب',
        // Europe
        'Warsaw': 'وارسو',
        'Prague': 'براغ',
        'Bratislava': 'براتيسلافا',
        'Budapest': 'بودابست',
        'Ljubljana': 'ليوبليانا',
        'Zagreb': 'زغرب',
        'Sarajevo': 'سراييفو',
        'Belgrade': 'بلغراد',
        'Podgorica': 'بودغوريتسا',
        'Tirana': 'تيرانا',
        'Skopje': 'سكوبيه',
        'Sofia': 'صوفيا',
        'Bucharest': 'بوخارست',
        'Chișinău': 'كيشيناو',
        'Kyiv': 'كييف',
        'Minsk': 'مينسك',
        'Vilnius': 'فيلنيوس',
        'Riga': 'ريغا',
        'Tallinn': 'تالين',
        'Helsinki': 'هلسنكي',
        'Stockholm': 'ستوكهولم',
        'Oslo': 'أوسلو',
        'Copenhagen': 'كوبنهاغن',
        'Reykjavik': 'ريكيافيك',
        'Dublin': 'دبلن',
        'Amsterdam': 'أمستردام',
        'Brussels': 'بروكسل',
        'Luxembourg': 'لوكسمبورغ',
        'Bern': 'برن',
        'Vienna': 'فيينا',
        'Vaduz': 'فادوتس',
        'Monaco': 'موناكو',
        'Andorra la Vella': 'أندورا لا فيلا',
        'San Marino': 'سان مارينو',
        'Vatican City': 'مدينة الفاتيكان',
        'Valletta': 'فاليتا',
        'Nicosia': 'نيقوسيا',
        'Athens': 'أثينا',
        'Lisbon': 'لشبونة',
        // Americas
        'Buenos Aires': 'بوينس آيرس',
        'Santiago': 'سانتياغو',
        'Montevideo': 'مونتيفيديو',
        'Asunción': 'أسونسيون',
        'Sucre': 'سوكري',
        'Lima': 'ليما',
        'Quito': 'كيتو',
        'Bogotá': 'بوغوتا',
        'Caracas': 'كاراكاس',
        'Georgetown': 'جورجتاون',
        'Paramaribo': 'باراماريبو',
        'Guatemala City': 'مدينة غواتيمالا',
        'Belmopan': 'بلموبان',
        'San Salvador': 'سان سلفادور',
        'Tegucigalpa': 'تيغوسيغالبا',
        'Managua': 'ماناغوا',
        'San José': 'سان خوسيه',
        'Panama City': 'مدينة بنما',
        'Kingston': 'كينغستون',
        'Port-au-Prince': 'بورت أو برنس',
        'Santo Domingo': 'سانتو دومينغو',
        'Havana': 'هافانا',
        'Nassau': 'ناساو',
        // Oceania
        'Wellington': 'ويلينغتون',
        'Suva': 'سوفا',
        'Port Vila': 'بورت فيلا',
        'Honiara': 'هونيارا',
        'Apia': 'آبيا',
        'Nuku\'alofa': 'نوكو ألوفا',
        'Tarawa': 'تاراوا',
        'Funafuti': 'فونافوتي',
        'Yaren': 'يارين',
        'Ngerulmud': 'نغيرولمود',
        'Majuro': 'ماجورو',
        'Palikir': 'باليكير',
        // Territories
        'Nuuk': 'نووك',
        'San Juan': 'سان خوان',
        'Stanley': 'ستانلي',
        'Port-aux-Français': 'بورت أو فرانسيه',
        'El Aaiún': 'العيون'
      }
    }
  },
  en: {
    translation: {
      title: "Interactive D3 Globe",
      subtitle: "A responsive, accessible globe with all world countries.",
      country: "Country",
      capital: "Capital",
      'globe.title': 'World Countries Globe',
      'globe.instructions': 'Drag to rotate. Scroll to zoom. Hover a country to see its name and capital.',
      'globe.loading': 'Loading map...',
      'globe.capital': 'Capital',
      'globe.unknown': 'Unknown',
      'search.placeholder': 'Search for a country...',
      'search.button': 'Search',
      'search.success': 'Country Found',
      'search.found': 'Successfully located the country on the globe.',
      'search.error': 'Country Not Found',
      'search.notFound': 'Could not find the specified country. Please try a different name.',
      'countryInfo.loading': 'Loading...',
      'countryInfo.capital': 'Capital',
      'countryInfo.population': 'Population',
      'countryInfo.area': 'Area',
      'countryInfo.languages': 'Languages',
      'countryInfo.currency': 'Currency',
      'countryInfo.region': 'Region',
      'countryInfo.unknown': 'Unknown',
     'countryInfo.km2': 'km²',
     'countryInfo.adminDivisions': 'Administrative divisions',
     'countryInfo.showDivisions': 'Show administrative divisions',
     'countryInfo.hideDivisions': 'Hide administrative divisions',
     'countryInfo.noDivisions': 'No administrative division data available for this country',
      'regions': {},
      'subregions': {},
      'languages': {},
      'currencies': {},
      'countries': {},
      'capitals': {}
    }
  }
};

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: 'ar', // Default language is Arabic
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false
    }
  });

export default i18n;